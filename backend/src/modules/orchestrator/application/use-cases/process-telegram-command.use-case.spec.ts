...
describe('ProcessTelegramCommandUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new ProcessTelegramCommandUseCase(mockRepository, ADMIN_CHAT_ID);
  });

  it('should reject commands from unauthorized chat IDs', async () => {
    const rawText = '/createpost test';
    
    await expect(useCase.execute('999999', rawText)).rejects.toThrow('Unauthorized chat ID');
  ...
  it('should ignore non-commands from the admin chat ID', async () => {
    ...
    await expect(useCase.execute(ADMIN_CHAT_ID, rawText)).rejects.toThrow('Not a valid command');
  });
});
